...
To call this MCP server as the requesting user instead of the app's service principal, declare the `apps` scope on the host app and pass the user-scoped `WorkspaceClient` into `DatabricksMCPServer`. See the **OBO** section in `SKILL.md` for the full pattern.
